...
class Position:
    def __init__(self, idx, ln, col, fn, ftxt):
        ...
        self.ln = ln
        self.col = col
        self.fn = fn  # self.case[0][0]: if 的 expr
        self.ftxt = ftxt
...
class CallNode:
    def __init__(self, node_to_call, arg_nodes):
        ...
        self.arg_nodes = arg_nodes  # 调用函数时传入的参数
...
class Function(Value):
    def __init__(self, name, body_node, arg_names):
        super().__init__()
        self.name = name or "<anonymous>"  # 若形参 name 为空，则将其赋值为匿名函数
        self.body_node = body_node
    ...
    def execute(self, args):
        res = RTResult()
        interpreter = Interpreter()  # 因函数体内部分变量拥有独立作用阈，故另设一个单独的解释器执行体内逻辑
        ...
        for i in range(len(args)):
            arg_name = self.arg_names[i]  # 参数名称
            arg_value = args[i]  # 参数值
            arg_value.set_context(new_context)  # 更新当前结点的上下文，便于报错时的定位处理
            new_context.symbol_table.set(
                arg_name, arg_value)  # 将形参的值传入被调用函数的符号表中

        value = res.register(interpreter.visit(
            self.body_node, new_context))  # 解释器的执行结果
        if res.error:
            return res
...
class Interpreter:
    ...
    def visit_CallNode(self, node, context):
        ...
        # 递归处理后，该语句最终会调用 visit_VarAccessNode 方法 依托结点名称（函数名称）于符号表内取得函数对象自身
        value_to_call = res.register(self.visit(node.node_to_call, context))
        # 因为 value_to_call 不是终结符，故需对 error 进行相关逻辑的处理
        if res.error:
            return res
